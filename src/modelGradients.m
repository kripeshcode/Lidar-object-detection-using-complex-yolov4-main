...
% (x, y, width, height, yaw), object confidence, and class probabilities. See the helper 
...
% Compute the loss.
boxLoss = loss.bboxOffsetLoss(YPredCell(:,[2 3 9 10 6 7]),boxTarget,objectMaskTarget,boxErrorScale);
objLoss = loss.objectnessLoss(YPredCell(:,1),objectnessTarget,objectMaskTarget);
clsLoss = loss.classConfidenceLoss(YPredCell(:,8),classTarget,objectMaskTarget);
